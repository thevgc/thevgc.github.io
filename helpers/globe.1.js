 ...
 */
var Globe = function Globe(container, urls, opts) {
  var PI = Math.PI;
  var PI_HALF = PI / 2;
  ...
  api.init = function() {
    ...
    earthGeometry = new THREE.SphereGeometry(200, 128, 128);

    // Light, reposition close to camera
    light = createMesh.directionalLight();
    // light = new THREE.PointLight(0xff0000, 1, 10000, 2);
  ...
  var createMesh = {
    ...
    earth: function(urls) {
      ...
      material.color = new THREE.Color("#007CBA")
      material.shininess = 1
      material.wireframe = false;
      // material.color = new THREE.Color("#007CBA");
      let loader = new THREE.TextureLoader();
      material.map = loader.load(urls.earth);

      if (urls.bump) {
        material.bump = loader.load(urls.bump);
        material.bumpScale = 0.02;
      }

      if (urls.specular) {
        material.specularMap = loader.load(urls.specular);
    ...
    atmosphere: function() {
      var material = new THREE.ShaderMaterial({
        vertexShader: [
          ...
          "void main() {",
          "vec4 color = vec4(vec3(0.0,1.0,1.0), 0.6);",
        ...
        fragmentShader: [
          ...
          // "attribute vec4 color;",
          "void main() {",
          // "vec4 color = vec4(vec3(1.0,1.0,1.0), 0.2);",
          "float intensity = pow( 0.8 - dot( vNormal, vec3( 0.0, 0.0, 1.0 ) ), 10.0 );",
  ...
  var checkAltituteBoundries = function() {
    ...
    // min zoom
    else if (distanceTarget > 900) distanceTarget = 900;
  };
